...
export const RoomExtrasSelector: React.FC<RoomExtrasSelectorProps> = ({
  ...
  const [selectedExtras, setSelectedExtras] = useState<RoomExtra[]>(extras.length ? extras : defaultExtras);
  ...
  return <div className="space-y-4">
      ...
      <div className="space-y-2">
        {selectedExtras.map(extra => <div key={extra.id} className="flex items-center justify-between border p-3 rounded-md">
            <div className="flex items-center">
              {getExtraIcon(extra.name)}
              <span>{extra.name} - {extra.price}€</span>
            </div>
            <div className="flex items-center space-x-2">
              <Button type="button" variant="outline" size="sm" onClick={() => handleQuantityChange(extra.id, -1)} disabled={extra.quantity === 0}>
                <Minus className="h-4 w-4" />
              </Button>
              <span className="w-8 text-center">{extra.quantity}</span>
              <Button type="button" variant="outline" size="sm" onClick={() => handleQuantityChange(extra.id, 1)}>
